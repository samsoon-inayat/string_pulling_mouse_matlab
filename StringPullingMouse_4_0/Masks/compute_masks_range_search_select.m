function mask = compute_masks_range_search_select(handles,frame,object,mc)
...
%     mc = getColors(handles,'Background',colorCols,intsct);%mouseColors(:,colorCols);
    if sm
        ...
        Im = getThisMask(hsvFrame,mc,nrows,ncols,radius);
    end
    mask = Im;
end

if strcmp(object,'body') || strcmp(object,'fur')
...
%     ec = getColors(handles,'Ears',colorCols,intsct);%earColors(:,colorCols);
    ec = mc;
    if sm
        Ie = getThisMask_KNN(hsvFrame,(ec),nrows,ncols,numberOfPoints);
    else
        Ie = getThisMask(hsvFrame,(ec),nrows,ncols,radius);
    end
    mask = Ie;
end
...
%     hc = getColors(handles,'hands',colorCols,intsct);%handColors(:,colorCols);
    ...
    if sm
        ...
        Ih = getThisMask(hsvFrame,hc,nrows,ncols,radius);
    end
    mask = Ih;
end
...
%     sc = getColors(handles,'String',colorCols,intsct);
    ...
    if sm
        ...
        Is = getThisMask(hsvFrame,sc,nrows,ncols,radius);
    end
    mask = Is;
end
