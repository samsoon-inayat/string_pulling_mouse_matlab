function mouse_rise_fall_ParmsX(handles,M,out,yR,yL)
oyR = yR;
oyL = yL;
yR = smooth(yR);
yL = smooth(yL);
...
for bt = 1:length(bout)
    ...
    nyR = abs(yR(sig_len)-0);
    nyL = abs(yL(sig_len)-maxY);
    onyR = abs(oyR(sig_len)-0);
    onyL = abs(oyL(sig_len)-maxY);
    ...
    [aR, locR] = findpeaks(zs_nyR, 'MinPeakHeight',2);%,'MinPeakDistance',20);
...
%     CD = [CD comb_down]; % combined motion frames
    ...
    for tt = 1:length(locR)-1
        x = onyR(locR(tt):locR(tt+1)+1)*scale_dist;
...
%         plot(x-x(rise_Rmin))
    ...
    for tt = 1:length(locL)-1
        x = onyL(locL(tt):locL(tt+1)+1)*scale_dist;
        mi = find(x==max(x));
...
pdfFileName = sprintf('%s_%dX.pdf',mfilename,bt);
pdfFileName = [pwd '\FiguresForPaper\pdfs\' pdfFileName]
save2pdf(pdfFileName,gcf,600);
...
xtickangle(30);
set(gca,'TickDir','out','linewidth',1,'FontSize',7,'FontWeight','Bold');
pdfFileName = sprintf('%s_riseTimeFallTimeX.pdf',mfilename);
pdfFileName = [pwd '\FiguresForPaper\pdfs\' pdfFileName]
save2pdf(pdfFileName,gcf,600);
