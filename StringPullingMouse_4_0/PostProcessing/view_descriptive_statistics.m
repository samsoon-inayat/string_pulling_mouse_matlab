function view_descriptive_statistics(handles,ds,saveFileName)

% ypos = 9:-2:0
if ~iscell(saveFileName)
display_desc_stats(handles,ds,[saveFileName '_image_sequence'],101,9);  set(gcf,'Name','Image Sequence');
if ~isempty(ds.motion)
    display_desc_stats(handles,ds.motion,[saveFileName '_motion'],102,7);  set(gcf,'Name','Speed Frames');
end
else
    display_desc_stats(handles,ds,{saveFileName{1},[saveFileName{2} '_image_sequence']},101,9);  set(gcf,'Name','Image Sequence');
    if ~isempty(ds.motion)
        display_desc_stats(handles,ds.motion,{saveFileName{1},[saveFileName{2} '_motion']},102,7);  set(gcf,'Name','Speed Frames');
    end
end
...
set(ff.hf,'Position',[9 ypos 6.9 1.25],'MenuBar','None','ToolBar','None');
set(gcf,'color','w');

titles = {'Mean','Median','Mode','Std. Dev.','Skewness','Kurtosis'};
