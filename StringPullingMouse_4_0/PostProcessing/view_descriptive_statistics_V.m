function view_descriptive_statistics_V(handles,ds,saveFileName)

% ypos = 9:-2:0
if ~iscell(saveFileName)
display_desc_stats(handles,ds,[saveFileName '_image_sequence_V'],101,9);  set(gcf,'Name','Image Sequence');
if ~isempty(ds.motion)
    display_desc_stats(handles,ds.motion,[saveFileName '_motion_V'],102,7);  set(gcf,'Name','Motion');
end
else
    display_desc_stats(handles,ds,{saveFileName{1},[saveFileName{2} '_image_sequence_V']},101,9);  set(gcf,'Name','Image Sequence');
    if ~isempty(ds.motion)
        display_desc_stats(handles,ds.motion,{saveFileName{1},[saveFileName{2} '_motion_V']},102,7);  set(gcf,'Name','Motion');
    end
end
...
titles = {'Std. Dev.','Skewness','Kurtosis'};
vars = {'standard_deviation','skewness','kurtosis'};
...
for cc = 1:size(groupIndices,2)
        for rr = 1:size(groupIndices,1)
            ...
            text(12,-19,titles(rr,cc),'FontSize',9,'FontWeight','normal');
            set(gca,'FontSize',7);
