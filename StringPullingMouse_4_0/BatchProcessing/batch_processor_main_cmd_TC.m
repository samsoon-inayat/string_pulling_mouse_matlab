...
mainFolder = 'E:\Users\samsoon.inayat\OneDrive - University of Lethbridge\Data\Hindlimb_Clasping\Surjeet';
pdfFolder = 'E:\Users\samsoon.inayat\OneDrive - University of Lethbridge\Data\Hindlimb_Clasping\Surjeet\pdfs';

dataFolders = {''};
metaFiles = {'Frames.mat'};
dii = 1;
data_folder = fullfile(mainFolder,dataFolders{dii});
filename = fullfile(data_folder,metaFiles{dii});

file_list = load(filename);
vid_files = file_list.Frames;

for ii = 1:length(vid_files)
    files_to_process{ii} = vid_files(ii).filename;
end
...
if setEpochs
    for ii = 1:length(vid_files)
        ...
        data = {vid_files(ii).StartStop(1),vid_files(ii).StartStop(2)};
