...
if nargin>3, Repeats=cell2mat(varargin(3)); else, Repeats=5; end
ToTest = m;
...
%     [~,~,dist]=kmeans(X,c,'emptyaction','drop'); %compute the sum of intra-cluster distances
    [~,~,dist] = kmeans(X,c,'emptyaction','drop','Options',options,'distance','sqEuclidean','Replicates',6);
...
[IDX,C,SUMD] = kmeans(X,K,'emptyaction','drop','Options',options,'distance','sqEuclidean','Replicates',6);
