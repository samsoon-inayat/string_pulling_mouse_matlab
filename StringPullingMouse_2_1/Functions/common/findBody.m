...
%                 LiaP = LiaP | ismember(M.P(:,[1 2]),[fn tag(1)],'rows');
                displayMessage(handles,sprintf('Erasing %s ... Processing frame %d - %d/%d ... time remaining %s','body',fn,ii,length(frameNums),getTimeRemaining(length(frameNums),ii)));
            end
            M.R(LiaR,:) = [];
%             M.P(LiaP,:) = [];
        end
...
%     else
    ...
    if sum(tMasks.Im(:)) == 0
        displayMessageBlinking(handles,sprintf('Can not find %s ... No mask found ... Processing frame %d - %d/%d ... time remaining %s','Body',fn,ii,length(frameNums),getTimeRemaining(length(frameNums),ii)),{'ForegroundColor','r'},3);
        continue;
    end
%     end
%     tMasks = get_masks_KNN(handles,fn);
...
%         P = [P;[ones(size(pixelsI))*fn ones(size(pixelsI))*tag(1) pixelsI]];
        ...
        saveRegionsMask(handles,C,fn,masksMap{1});
    else
