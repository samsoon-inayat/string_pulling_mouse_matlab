...
% if ~get(handles.checkbox_calculate_masks_again,'Value')
    ...
    oMasks.In = reshape(btMask(:,4),zw(4)-zw(2)+1,zw(3)-zw(1)+1);
    oMasks.Is = reshape(btMask(:,5),zw(4)-zw(2)+1,zw(3)-zw(1)+1);
%     if fn > 1
%         oMasks.bd = reshape(btMask(:,6),zw(4)-zw(2)+1,zw(3)-zw(1)+1);
%         oMasks.fd = reshape(btMask(:,7),zw(4)-zw(2)+1,zw(3)-zw(1)+1);
%     end
% else
%     global frames;
%     thisFrame = frames{fn};
