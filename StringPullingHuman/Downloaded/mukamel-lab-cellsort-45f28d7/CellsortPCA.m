...
% end
dsamp = [1,1];
fnmat = fullfile(outputdir,'pca_ica.mat');
if ~isempty(dir(fnmat))
    display(sprintf('CELLSORT: Movie %s already processed;',fn));
    forceload = input(' Re-load data? [0-no/1-yes] ');
    if ~isempty(forceload) && forceload==1
        load(fnmat)
        return
    end
end
...
%         end
        mov = fn;
        fprintf(' Read frame %4.0f out of %4.0f; ', nt, nt)
        toc
        mov = reshape(mov, npix1, nt);
